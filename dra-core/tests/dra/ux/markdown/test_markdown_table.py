...
from dra.core.ux.markdown.elements import MarkdownTable
...
class TestMarkdownTable(unittest.TestCase):
    ...
    def make_columns_justifications(self, columns: list[str], justs_samples: list[str]) -> tuple[list[tuple[str,str]],list[str]]:
    ...
    @given(
        ...
        justifications(min_size=10, max_size=20))
    def test_justify_with_valid_values(self, columns: list[str], justs_samples: list[str]):
        """
    ...
    def test_make_table_with_columns_with_justifications(self, 
        title: str, columns: list[str], justs_samples: list[str]):
        """
        ...
        table = MarkdownTable(title=title, columns_with_justifications = cjs)
        self.assert_header_rows(table, title, columns, justs, [])

    def test_make_table_fails_if_both_columns_and_columns_with_justifications_nonempty(self):
        """
        Verify that table construction fails if both `columns` and `columns_with_jutifications` 
        are nonempty.
        """
        with self.assertRaises(ValueError):
             MarkdownTable(title='oops!', columns = ['one'], columns_with_justifications = [('two', 'left')])

    @given(st.lists(no_linefeeds_text()))
    def test_table_add_columns_that_default_to_left_justification(self, columns: list[str]):
        """
        Verify that add_columns with a list of columns is properly formed.
        """
        table = MarkdownTable()
    ...
    @given(
        st.lists(no_linefeeds_text()),
        justifications(min_size=10, max_size=20))
    def test_table_add_columns_with_justifications(self,
        columns: list[str],
        justs_samples: list[str]):
        """
        Verify that add_columns_with_justifications with a list of (column, justification)
        pairs is properly formed.
        """
        table = MarkdownTable()
        cjs, justs = self.make_columns_justifications(columns, justs_samples)
        table.add_columns_with_justifications(cjs)
        self.assert_header_rows(table, '', columns, justs, [])
        more_columns = [f"{col}2" for col in columns]
        more_cjs, more_justs = self.make_columns_justifications(more_columns, justs_samples)
        table.add_columns_with_justifications(more_cjs)
        self.assert_header_rows(table, '', columns+more_columns, justs+more_justs, [])

    @given(
        st.lists(no_linefeeds_text()),
        justifications(min_size=10, max_size=20),
        st.lists(no_linefeeds_nonempty_text(), min_size=1),
        st.integers(min_value = 1, max_value=10))
    def test_table_add_row_with_list_of_cells(self, 
        columns: list[str],
        justs_samples: list[str],
        ...
        """
        cjs, justs = self.make_columns_justifications(columns, justs_samples)
        table = MarkdownTable(columns_with_justifications = cjs)
    ...
    @given(
        st.lists(no_linefeeds_nonempty_text()), 
        justifications(min_size=10, max_size=20), 
        st.lists(no_linefeeds_nonempty_text(), min_size=1), 
        st.integers(min_value = 1, max_value=10))
    def test_table_add_row_with_dict(self, 
        columns: list[str],
        justs_samples: list[str],
        cell_samples: list[str],
        num_rows: int):
        ...
        if len_cols > 0:
            ...
            for j in range(num_rows):
                ...
                table.add_row_with_dict(row)
                rows.append(row)
        rows_lists = []
        for row in rows:
            rows_lists.append([row.get(col, '') for col in columns])
    ...
    def test_table_add_row_with_dict_with_bad_keys_fails(self, columns: list[str], other_text: str):
        ...
        if not other_text in set_columns:
            with self.assertRaises(ValueError):
                table.add_row_with_dict({other_text: 0})
