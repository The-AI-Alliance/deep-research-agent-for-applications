...
class ParserUtil():
    ...
    def get_default_argument(self, key: str, default: Optional[Any] = None) -> Optional[Any]:
        if default:
            return default
        else:
    ...
    def make_def_mcp_agent_config_path(self, provider: Optional[str] = None) -> str:
    ...
    def add_arg_markdown_research_report_title(self, default: Optional[str] = None):
        default = self.get_default_argument("--report-title", default)
        self.parser.add_argument(
            "--report-title", default=default,
            help=f"A concise title to use for the report. If None, you will be prompted to input it."
        )

    def add_arg_markdown_report_path(self, default: Optional[str] = None):
        default = self.get_default_argument("--markdown-report", default)
        self.parser.add_argument(
    ...
    def add_arg_output_dir(self, default: Optional[str] = None):
        default = self.get_default_argument("--output-dir", default)
        self.parser.add_argument(
            "--output-dir", default=default,
            help=f"Path where Excel and other output files will be saved. (Default: {default})"
        )

    def add_arg_templates_dir(self, default: Optional[str] = None):
        default = self.get_default_argument("--templates-dir", default)
        self.parser.add_argument(
    ...
    def add_arg_markdown_yaml_header_template_path(self, default: Optional[str] = None):
        default = self.get_default_argument("--markdown-yaml-header", default)
        self.parser.add_argument(
    ...
    def add_arg_research_model(self, default: Optional[str] = None):
        default = self.get_default_argument("--research-model", default)
        self.parser.add_argument(
    ...
    def add_arg_provider(self, default: Optional[str] = None):
        default = self.get_default_argument("--provider", default)
        self.parser.add_argument(
    ...
    def add_arg_temperature(self, default: Optional[float] = None):
        default = self.get_default_argument("--temperature", default)
        self.parser.add_argument(
    ...
    def add_arg_max_iterations(self, default: Optional[int] = None):
        default = self.get_default_argument("--max-iterations", default)
        self.parser.add_argument(
    ...
    def add_arg_max_tokens(self, default: Optional[int] = None):
        default = self.get_default_argument("--max-tokens", default)
        self.parser.add_argument(
    ...
    def add_arg_max_cost_dollars(self, default: Optional[float] = None):
        default = self.get_default_argument("--max-cost-dollars", default)
        self.parser.add_argument(
    ...
    def add_arg_max_time_minutes(self, default: Optional[int] = None):
        default = self.get_default_argument("--max-time-minutes", default)
        self.parser.add_argument(
    ...
    def add_arg_mcp_agent_config_path(self, default: Optional[str] = None):
        default = self.get_default_argument("--mcp-agent-config", default)
        self.parser.add_argument(
    ...
    def __to_valid_file_name(self, s: str) -> str:
        """Replace characters that are not allowed in file names!"""
        return re.sub('[:/]', ' - ', s)

    def _determine_report_path(self, output_dir_path: Path, research_report_title: Optional[str] = None) -> Path:
        """
        ...
        """
        mr = self.processed_args.get('markdown_report')
        if not mr:
    ...
    def process_args(self) -> MutableMapping[str, Any]:
        ...
        observers_d: Mapping[str,Observer] = {}
    ...
    def only_verbose(self, formatter: str = 'str') -> str:
        return formatter if self.processed_args.get('verbose') else ''

    def common_variables(self) -> list[Variable]:    
        return [
            Variable("provider",                   self.processed_args['provider'], kind='provider'),
            Variable("research_model",             self.processed_args['research_model'], kind='code'),
    ...
    def only_verbose_common_vars(self) -> list[Variable]:
        ...
        return [
            Variable("verbose",           self.processed_args['verbose'], kind=fmt),
            Variable("short_run",         self.processed_args['short_run'], kind=fmt),
...
class Runner():
    ...
    def __init__(self,
        ...
        self.config: DeepOrchestratorConfig = DeepResearch.make_default_config(
            self.parser_util.processed_args['short_run'],
        ...
        self.deep_research = DeepResearch(
            ...
            provider=self.parser_util.processed_args['provider'],
            config=self.config,
            tasks=self.tasks,
