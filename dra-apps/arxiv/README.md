# Deep Research Agent for ArXiv Research
...
The easiest way to run it with default values for all optional arguments is `make app-run-arxiv`.

The `app-run-arxiv` target does some setup and then runs the command `cd dra-apps && uv run -m arxiv.main ...` where `...` is a lot of arguments. 
...
>
> ```shell
> make.sh --short-run app-run-arxiv
...
usage: main.py [-h] [-q QUERY] [--categories CATEGORIES]
               ...
               [--arxiv-research-prompt-path ARXIV_RESEARCH_PROMPT_PATH]
...
ArXiv Deep Research using orchestrated AI agents
...
options:
  ...
  --markdown-report MARKDOWN_REPORT
                        ...
                        (Default: arxiv_research_report.md) If the path
  ...
  --arxiv-research-prompt-path ARXIV_RESEARCH_PROMPT_PATH
                        ...
                        located. (Default: arxiv_research_agent.md) If the
  ...
  --mcp-agent-config MCP_AGENT_CONFIG
                        ...
                        dra/apps/arxiv/config/mcp_agent.config.yaml) Specify
...
cd dra-apps && uv run -m arxiv.main \
    ...
    --templates-dir "arxiv/templates" \
    --arxiv-research-prompt-path "arxiv_research_agent.md" \
    --research-model "gpt-4o" \
    --provider "openai" \
    --mcp-agent-config "arxiv/config/mcp_agent.config.yaml" \
    --temperature 0.7 \
    --max-iterations 25 \
    --max-tokens 500000 \
...
Finally, `--arxiv-research-prompt-path`, is also unique to this app. It is the path to the prompt template file. The default value for this path is `arxiv_research_agent.md`. Because a directory path isn't specified, this file will be searched for in the value passed with `--templates-dir`, which defaults to `dra/apps/arxiv/templates`.
