...
1. **Find ArXiv Papers**: Search ArXiv for the most relevant papers, by examining the paper titles and abstracts. If any subjects are provided, focus your search in those subject areas within ArXiv.
...
1. **Use Docling to Parse PDFs**: When PDFs are downloaded use the `docling` MCP server to parse them.
1. **Read and Analyze the Papers**: Read and analyze the downloaded or parsed papers to determine their relevancy and utility for the research query.
...
**Documentation Requirements**: For every result, record the source URL, author, publisher (especially for content outside of ArXiv), title, date, and pinpoint location. Keep direct quotes ≤ 100 words.
...
1. **Summary**: A summary of the resource's information on the topic. Where very specialized technical jargon and acronyms are used in the summary, explain their meaning concisely.
...
One or more quotes from the paper.
```
